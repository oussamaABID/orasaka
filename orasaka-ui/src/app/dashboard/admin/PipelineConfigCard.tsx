      ...
      <section className="space-y-1.5">
        {interceptors.map((item, idx) => (
          <div
            ...
            aria-label={`Interceptor ${item.interceptorKey}`}
            draggable
            ...
            className={`group flex items-center gap-3 p-3 rounded-xl border transition-all duration-200 cursor-grab active:cursor-grabbing ${(() => {
              if (dragIdx === idx) return "border-amber-400 dark:border-amber-600 bg-amber-50/50 dark:bg-amber-950/20 shadow-lg scale-[1.01]";
              if (item.enabled) return "border-card-border bg-background hover:border-zinc-300 dark:hover:border-zinc-600";
              return "border-card-border bg-zinc-50/50 dark:bg-zinc-900/50 opacity-60";
            })()}`}
          >
            {/* Drag handle */}
