...
export const JobRow: React.FC<JobRowProps> = ({
  ...
  return (
    <React.Fragment>
        ...
        <td className="p-4 font-medium text-[var(--text-secondary)] text-[11px]">
          {job.status === "PENDING" || job.status === "PROCESSING" ? (
            <section className="space-y-1">
              ...
              {/* Mini progress bar */}
              <figure className="h-1 w-16 rounded-full bg-[var(--surface-3)] overflow-hidden">
                <figure className="h-full w-1/2 rounded-full bg-blue-500 animate-pulse" />
              </figure>
            </section>
          ) : (
            getDuration(job)
      ...
      {job.status === "FAILED" && isExpandedError && job.errorMessage && (
