...
export const ChatInputBar: React.FC<ChatInputBarProps> = ({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="w-full px-4 pb-5 md:pb-7 pt-0 bg-transparent flex flex-col items-center relative z-25"
    >
      <section className="glass-card max-w-3xl w-full shadow-xl rounded-xl p-3 flex flex-col gap-2 transition-[border-color,box-shadow] duration-300 focus-within:border-[var(--accent)] focus-within:shadow-[var(--accent-glow)]">
        <ChatIndicators
        ...
        <section className="flex gap-3 items-end relative">
          ...
          <button
            ...
            className="p-2.5 rounded-lg bg-[var(--surface-2)] hover:bg-[var(--surface-3)] border border-[var(--border-subtle)] text-[var(--text-secondary)] transition-colors duration-150 flex items-center justify-center flex-shrink-0 mb-0.5"
          ...
          <section className="flex-1 relative flex items-end">
            <button
              ...
              className="absolute left-3 bottom-3 p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors duration-150 z-10"
            ...
            <textarea
              ...
              rows={1}
              className="w-full pl-10 pr-24 py-3 bg-transparent border-0 text-[var(--text-primary)] placeholder:text-[var(--text-muted)] rounded-lg focus:outline-none focus:ring-0 resize-none overflow-hidden text-sm leading-relaxed"
              style={{
                minHeight: "44px",
                maxHeight: "140px",
                height: "auto",
            ...
            <section className="absolute right-2 bottom-1.5 flex items-center gap-2">
              ...
              <Button
                type="submit"
                disabled={isDisabled}
                className="px-4 py-1.5 h-8 text-xs font-semibold rounded-lg group"
              >
                {isSending ? (
                  t.chat.sending
                ) : isGenerating ? (
