            ...
            <div className="max-h-64 overflow-y-auto py-1 scrollbar-thin">
              {lastJobs.length === 0 ? (
                ...
                <ul className="divide-y divide-zinc-100 dark:divide-zinc-800/40">
                  {lastJobs.map((job) => {
                    const featureName = job.featureKey.includes(
                      MODEL_CATEGORY.VIDEO,
                    )
                      ? t.notifications.videoGen
                      : job.featureKey.includes(MODEL_CATEGORY.IMAGE)
                        ? t.notifications.imageGen
                        : job.featureKey.includes(MODEL_CATEGORY.SPEECH)
                          ? t.notifications.speechGen
                          : t.notifications.textGen;
                    return (
                      <li
                        key={job.id}
