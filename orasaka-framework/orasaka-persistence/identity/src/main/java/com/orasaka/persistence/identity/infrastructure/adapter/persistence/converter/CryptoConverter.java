...
public class CryptoConverter implements AttributeConverter<String, String> {
   ...
   *
   * @throws IllegalStateException if {@code crypto.key} or {@code crypto.salt} is missing
   */
  public CryptoConverter() {
    ...
    if (key == null || key.isBlank()) {
      throw new IllegalStateException(
          "CRYPTO_KEY environment variable must be set for AES-256 encryption");
    }
    if (salt == null || salt.isBlank()) {
      throw new IllegalStateException("CRYPTO_SALT environment variable must be set (16-char hex)");
    }
