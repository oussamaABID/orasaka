...
class EngineArchitectureTest {
  ...
  @Test
  @DisplayName(
      "All controllers reside in com.orasaka.gateway.infrastructure.adapter.. and never bypass the OrchestrationPipeline layer")
  void controllersResideInEndpointAndDoNotBypassPipeline() {
    ...
    // Assert that controllers never bypass OrchestrationPipeline/AiClient/GraphEngine to call
    // models directly
    noClasses()
        .that()
        .resideInAPackage("com.orasaka.gateway.infrastructure.adapter..")
        ...
        .because(
            "Controllers must not bypass the OrchestrationPipeline/AiClient layer to interact with the engine directly")
        .check(allAppClasses);
  }
