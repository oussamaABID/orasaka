...
export function useNodeCardState(node: OperationNode, onExecuted?: () => void) {
  ...
  const globalResult = (playgroundResults[node.id] || null) as { success: boolean; data: string; error?: string } | null;
  ...
  const executeNode = async () => {
    ...
    if (template) {
      ...
      for (const raw of rawPlaceholders) {
        const colonIdx = raw.indexOf(":");
        const fieldName = colonIdx === -1 ? raw : raw.substring(0, colonIdx);
  ...
  const isJobRunning = activeJob
    ? activeJob.status === JOB_STATUS.PENDING || activeJob.status === JOB_STATUS.PROCESSING
    : false;
