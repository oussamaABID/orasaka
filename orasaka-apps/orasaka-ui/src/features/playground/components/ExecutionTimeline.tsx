...
export function ExecutionTimeline({
  ...
  const steps = [1, 2, 3, 4].map((id) => {
    ...
    const title = String(t.executionTimeline[titleKey]);
    let desc = String(t.executionTimeline[descKey]);
    if (id === 2) {
      desc = desc.replaceAll("{modelName}", modelName || "default");
    }
    return {
      id,
      title,
            ...
            >
              ...
              <div className="absolute -left-[31px] top-0.5 bg-[var(--surface-1)] rounded-full p-0.5">
                {(() => {
                  ...
                  if (status === "active") return <Loader2 className="h-4.5 w-4.5 text-amber-500 animate-spin" />;
                  return <Circle className="h-4.5 w-4.5 text-[var(--text-muted)]" />;
                })()}
              </div>
              ...
              <div className="flex flex-col gap-0.5">
                <span
                  className={`text-xs font-semibold ${(() => {
                    if (status === "completed") return "text-[var(--text-muted)] line-through decoration-[var(--border-subtle)]";
                    if (status === "active") return "text-[var(--text-primary)] font-bold";
