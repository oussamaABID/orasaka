...
public final class ContextResolver {
  ...
  public static Context resolve(
    ...
    UserProfile profile = userProfileProvider != null
        ? userProfileProvider.getProfile(user.id().toString())
        : null;
    return ContextMapper.buildContext(user, conversationId, UUID.randomUUID(), profile);
  }
}
