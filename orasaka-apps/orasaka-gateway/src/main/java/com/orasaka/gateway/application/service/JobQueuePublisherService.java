...
public class JobQueuePublisherService {
  ...
  private String resolveRoutingKey(JobMessage message) {
    ...
    if (userId != null && featureKey != null) {
      try {
        return jdbcClient.sql("SELECT r.target_routing_key FROM orasaka_routing_rules r " +
        ...
        logger.warn("Failed to resolve dynamic routing key from database, falling back to default", e);
      }
    }
  ...
  private String getDefaultRoutingKey(String featureKey) {
    if (featureKey != null && (featureKey.contains("IMAGE") || featureKey.contains("VIDEO") || featureKey.contains("MEDIA"))) {
