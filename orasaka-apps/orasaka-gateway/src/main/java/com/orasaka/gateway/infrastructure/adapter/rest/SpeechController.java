...
/** REST controller for executing text-to-speech tasks. */
@RestController
...
public class SpeechController {
  ...
  /**
   * Constructs the speech controller.
   *
  ...
  /** Request DTO for Speech synthesis parameters. */
  public static record SpeechGenerationRequest(String text, String model, String voice) {
    public SpeechGenerationRequest {
      Objects.requireNonNull(text, "Text parameter is required");
    }
  }

  /**
   * REST endpoint to submit a text-to-speech task.
   *
   * @param request The speech request parameters.
  ...
  public ResponseEntity<Map<String, Object>> generateSpeech(
      @RequestBody SpeechGenerationRequest request, @AuthenticationPrincipal User user) {
    logger.debug("Received speech synthesis request for text length: {}", request.text().length());
    String userId = user.id().toString();

    String activeModel =
        MediaJobOrchestrator.resolveModel(
            request.model(), "speech", "piper-en-medium-ryan", catalogModelService);
    MediaJobOrchestrator.validateModel(activeModel, "speech", modelCatalogProperties);

    Map<String, Object> payload = new HashMap<>();
    payload.put("prompt", request.text());
    payload.put("voice", request.voice() != null ? request.voice() : "ryan");
    payload.put("model", activeModel);

    return MediaJobOrchestrator.submitJob(
        userId,
        "orasaka.core.media.speech",
        activeModel,
        payload,
        uploadDirProperty,
        jobService,
