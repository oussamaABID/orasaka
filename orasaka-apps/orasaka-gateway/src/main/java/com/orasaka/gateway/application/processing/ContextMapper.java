...
final class ContextMapper {
   ...
   * @param sessionId        The explicit inbound request session tracking token (non-null) to maintain cross-service traceability.
  ...
  public static Context buildContext(
    ...
    Set<Authority> authorities = user.authorities().stream()
        .map(Authority::new)
    ...
    // Transitive payload: request-bound sessionId passed from interceptor for cross-service traceability
    ...
    if (profile != null) {
      ...
      if (profile.primaryIndustry() != null) profileContext.put("primaryIndustry", profile.primaryIndustry());
