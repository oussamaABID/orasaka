 ...
 */
export function CredentialsSection({ fetchHeaders }: Readonly<CredentialsSectionProps>) {
  ...
  const handleSave = async (providerName: string, apiKey: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Failed to save credential for ${providerName}`);
      }
    } catch {
  ...
  const handleDelete = async (providerName: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        throw new Error(`Failed to delete credential for ${providerName}`);
      }
    } catch {
      setMessage(t.errors.generic);
    } finally {
