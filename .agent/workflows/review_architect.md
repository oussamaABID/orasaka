...
## §0 Init
1. Load `.agent/rules/governance_boundary.md`
2. Load `.agent/rules/naming_conventions.md`
3. Load `.agent/rules/security_standards.md`
4. Load `.agent/rules/performance_standards.md`
5. Load `.agent/rules/ui_standards.md`
...
- **Frontend**: `npm run format` / `npm run lint` inside `orasaka-apps/orasaka-ui` (workspace root)
- **Python**: `ruff check orasaka-apps/orasaka-workers/ --fix`, `black orasaka-apps/orasaka-workers/`
...
6. SonarCloud: `./mvnw verify sonar:sonar -Dsonar.token=$SONAR_TOKEN`.
