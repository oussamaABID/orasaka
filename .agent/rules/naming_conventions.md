...
- **orasaka-ui**: Components: `PascalCase`. Hooks: `camelCase` (prefixed with `use`). Constants: `kebab-case.constants.ts`. APIs: `kebab-case.api.ts`. Tests: `*.test.tsx`. i18n keys: `dot.separated.camelCase`.
- **orasaka-apps/orasaka-workers/video-worker (Python)**: `snake_case` (fn/var/module), `PascalCase` (classes), `UPPER_SNAKE_CASE` (constants). Type hints required on public APIs.
