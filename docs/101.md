...
Orasaka uses [SonarCloud](https://sonarcloud.io) for continuous code quality inspection. The entire monorepo (Java, TypeScript, Python) is analyzed under a single unified project key.

### 10.1 Project Key

| Project | Languages | SonarCloud Key | Dashboard |
|---------|-----------|----------------|-----------|
| **orasaka-monorepo** | Java, TS, Python | `oussamaABID_orasaka` | [View →](https://sonarcloud.io/dashboard?id=oussamaABID_orasaka) |

### 10.2 Prerequisites

```bash
# Set the SONAR_TOKEN environment variable in .env
# SONAR_TOKEN=your_token_here
```

> ⚠️ **Never hardcode the token.** It is injected automatically via the `.env` file when running the scan script.

### 10.3 Running Analysis

To analyze the entire repository at once, use the unified script provided at the root of the project:
...
This script will:
1. Build the Java backend (`mvn clean verify`) to generate compiled `.class` files and JaCoCo coverage reports.
2. Execute a single `npx sonar-scanner` run using the root `sonar-project.properties` to upload all Java, TS, and Python code to SonarCloud.

### 10.4 Configuration File

| File | Location | Purpose |
|------|----------|---------|
| `sonar-project.properties` | `(root)` | Global configuration targeting all source directories, binaries, and exclusions |
