...
```bash
# First time setup
npx orasaka init                  # Detect workspace, generate .env
npx orasaka install               # Verify tools, configure video worker venv

# Daily development
npx orasaka start --mode dev      # Start middleware + AI engines
npx orasaka dev                   # Launch the full stack in parallel
...
# Monitoring
npx orasaka status                # Service health dashboard
