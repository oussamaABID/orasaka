...
graph TD
    ...
    Mobile["orasaka-mobile-client (Expo :8081)"] -->|BFF Proxy| GW
...
### Context-Matrix Pipeline (Canonical)
1. **UserContextResolver**: User profiles & RBAC roles.
2. **SystemContextInjector**: Environment signals & active capabilities.
3. **LanguageAlignmentInterceptor**: English reasoning alignment.
4. **MemoryInterceptor**: FIFO conversation history prepend.
5. **RagInterceptor**: Tenant-isolated vector store RAG context.
6. **McpInterceptor**: Resolve external MCP server tools/data.
7. **RefinerInterceptor**: Resolves contextual refinement (AI-dependent).
8. **RouterInterceptor**: Routes to the optimal model based on intent (AI-dependent).
9. **ToolInterceptor**: Dynamic tool registration & callbacks.
10. **CostShieldInterceptor**: Switches to cloud fallback if memory usage exceeds 85%.
11. **QuantumValidationAdvisor**: 4-tier closed-loop validation (A/B/C/D).
...
| `orasaka-web-client` | `orasaka-apps/orasaka-ui/orasaka-web-client/` | Next.js 16 web client with cinematic dark mode and input-blocking. |
| `orasaka-web-admin` | `orasaka-apps/orasaka-ui/orasaka-web-admin/` | SecOps Administration Console on port 3001. |
...
| `orasaka-end2end` | `orasaka-end2end/` | Hermetic E2E: Java/Web Gateway API validation only. Mobile out of scope. |
