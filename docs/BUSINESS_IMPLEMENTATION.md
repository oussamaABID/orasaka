...
graph TD
    ...
    lang --> memory["MemoryInterceptor"]
    memory --> rag["RagInterceptor"]
...
mvn clean install -pl orasaka-framework/orasaka-identity

# 2. Compile gateway API with dependencies
mvn clean compile -pl orasaka-apps/orasaka-gateway -am

# 3. Build CLI executable
npm run build --prefix orasaka-apps/orasaka-ui/orasaka-cli

# 4. Install UI workspace dependencies
npm install --prefix orasaka-apps/orasaka-ui
```
