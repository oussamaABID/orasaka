...
All variables are defined in the root `.env` file (loaded by `properties-maven-plugin` at the `initialize` phase). Frontend-specific variables are in `orasaka-apps/orasaka-ui/.env.local`.
...
### Frontend BFF (Next.js) — `orasaka-apps/orasaka-ui/.env.local`
