...
>
> **Scope**: The `orasaka-end2end` module validates **Java/Web Gateway API only**. Mobile (`orasaka-mobile-client`) is excluded from E2E — it runs lightweight TypeScript validation via the split-track CI pipeline.
...
| **E2E (CLI)** | Vitest / Execa | Sandboxed CLI subprocess testing | `orasaka-apps/orasaka-ui/orasaka-cli/e2e/` |
...
- Run (standalone): `npm run test:e2e --prefix orasaka-apps/orasaka-ui/orasaka-web-client`.
...
```
- Run (via Maven): `./mvnw clean verify -P e2e-tests`.
- Run (standalone): `npm run test:e2e --prefix orasaka-apps/orasaka-ui/orasaka-cli`.

---

## 6. Split-Track CI Pipeline

The CI pipeline uses `dorny/paths-filter` to separate backend/web validation from mobile validation:

```
Track A (always runs on push/PR):
[Phase 1: Lint / Spotless] ──► [Phase 2: Java Testcontainers] ─┬─► [Phase 5: SonarCloud]
                           ──► [Phase 3: JS/TS Unit Tests]   ─┘─► [Phase 4: Playwright E2E]

Track B (path-triggered: orasaka-mobile-client/, orasaka-shared/, workspace package.json):
[Mobile Validation: ESLint + tsc --noEmit]
```

> **Note**: `frontend-maven-plugin` in the root `pom.xml` runs `npm ci` and `npm run build` during the Maven lifecycle — web targets only. Mobile is explicitly excluded via `--ignore=orasaka-mobile-client`.
