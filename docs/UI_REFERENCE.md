...
The Orasaka platform includes a comprehensive, modern Next.js 16 frontend designed with cinematic dark-mode and glassmorphism. This document maps out the available interfaces, workflows, and their connections to the backend.
...
*   **Input Blocking:** To ensure stability, the UI automatically locks text inputs and buttons when a request `isGenerating` or `isSending`.
*   **Date Formatting:** All dates are formatted on the client using `date-fns` to avoid hydration mismatches.
