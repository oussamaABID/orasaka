...
class ToolCacheServiceTest {
  ...
  void get_localCacheHit_returnsValue() {
    ...
    service.put("tool1", "key1", "cached-value", 600);

    String result = service.get("tool1", "key1");
  ...
  void get_localCacheMiss_dbHit_returnsValue() {
    ...
    entity.setId(new ToolCacheId("tool1", "key2"));
    entity.setCacheValue("db-value");
    entity.setExpiresAt(Instant.now().plusSeconds(3600));

    when(cacheRepository.findById(new ToolCacheId("tool1", "key2")))
        .thenReturn(Optional.of(entity));

    String result = service.get("tool1", "key2");
  ...
  void get_localCacheMiss_dbMiss_returnsNull() {
    ...
    assertNull(service.get("tool1", "unknown"));
  }

  @Test
  void get_localCacheMiss_dbExpired_evictsAndReturnsNull() {
    ToolCacheEntity entity = new ToolCacheEntity();
    entity.setId(new ToolCacheId("tool1", "key3"));
    entity.setCacheValue("expired-value");
    entity.setExpiresAt(Instant.now().minusSeconds(10));

    when(cacheRepository.findById(new ToolCacheId("tool1", "key3")))
        .thenReturn(Optional.of(entity));

    String result = service.get("tool1", "key3");

    assertNull(result);
    verify(cacheRepository).deleteById(new ToolCacheId("tool1", "key3"));
  }
  ...
  void get_dbException_returnsNullGracefully() {
    ...
    String result = service.get("tool1", "keyErr");
  ...
  void put_storesInLocalAndDb() {
    service.put("tool1", "key1", "value", 300);

    verify(cacheRepository).save(any(ToolCacheEntity.class));
    assertEquals("value", service.get("tool1", "key1"));
  }
  ...
  void put_dbException_localCacheStillWorks() {
    ...
    service.put("tool1", "key-err", "value", 300);

    // Local cache should still have it
    assertEquals("value", service.get("tool1", "key-err"));
  }
  ...
  void get_localCacheExpired_evictsLocalAndDb() {
    ...
    service.put("tool1", "key-exp", "val", 0);

    // Should detect as expired
    String result = service.get("tool1", "key-exp");

    assertNull(result);
    verify(cacheRepository).deleteById(new ToolCacheId("tool1", "key-exp"));
  }
}
