 ...
 *
 * <p>Captures the minimal structural output produced by any {@code OAuth2ProviderVerifier}
 * after successfully validating an incoming identity token. This record is provider-agnostic
 * and acts as the canonical bridge between external identity systems and Orasaka's internal
 * user reconciliation pipeline.
 *
 * @param email The verified email address extracted from the provider token.
 * @param providerId The unique user identifier assigned by the external provider (e.g., Google
 *     sub, GitHub user ID).
 * @param name The display name or full name of the user from the provider profile.
 * @param avatarUrl The URL pointing to the user's avatar image from the provider profile.
