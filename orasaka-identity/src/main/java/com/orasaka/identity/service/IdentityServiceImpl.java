...
/**
 * Package-private concrete implementation of the {@link IdentityService} port interface.
 *
 * <p>Responsible for user lifecycle management: profile resolution, self-service registration,
 * preference persistence, password-based authentication, user interceptions, and token-based email
 * verification.
 *
...
class IdentityServiceImpl implements IdentityService {
   ...
   * {@inheritDoc}
   *
   * @throws BadCredentialsException if the user is not found, is a federated user (null password
   *     hash), or the password does not match.
   */
  @Override
  public User authenticate(String email, String password) {
    ...
    // Federated users have null password hashes — they cannot authenticate via credentials.
          ...
          transactionTemplate.execute(
              status -> {
                UserEntity userEntity =
                    UserMapper.toRegistrationEntity(
                        userId,
                ...
                if (verificationEnabled) {
                  ...
                  VerificationTokenEntity tokenEntity =
                      VerificationTokenMapper.toEntity(userId, tokenHash, expiry);
  ...
  /**
   * Triggers an interception block for the specified user. Internal-only — not exposed on the
   * {@link IdentityService} interface.
   *
