...
> _Use it to:_ Build personalized AI assistants, customer support bots, or internal knowledge agents.
...
> _Use it to:_ Let your AI fetch live stock prices, query databases, scrape websites, or call any API.
...
> _Use it to:_ Create product mockups, marketing visuals, concept art, or AI-generated thumbnails.
...
> _Use it to:_ Generate short-form video content, animated explainers, or creative prototypes.
...
> _Use it to:_ Build voice assistants, podcast generators, or accessible content pipelines.
...
> _Use it to:_ Build searchable knowledge bases, documentation Q&A, or internal wiki assistants.
...
> _Use it to:_ Build content moderation tools, video summarizers, or visual search engines.
...
> _Use it to:_ Build feature-flagged dashboards, tiered pricing UIs, or admin control panels.
...
graph TD
    ...
    gateway --> persistence["💾 orasaka-persistence"]
    core --> persistence
    ...
    class core,identity,tools,persistence lib;
    class gateway app;
    class ui,cli fe;
```
...
| **`orasaka-persistence`** | Decoupled Persistence & Infrastructure State Management. Exposes `JobPersistenceProvider` and RabbitMQ AMQP config. |
...
# Stop all workers
./ops/local/scripts/stop.sh

# Start Docker containers (PostgreSQL + pgvector, MCP debug server)
docker compose -p orasaka -f ops/local/docker/docker-compose.yml up -d
```
...
Orasaka provides a production-ready, automated Infrastructure-as-Code (IaC) setup driven by Terraform, orchestrating AWS, RunPod, and Modal.

For detailed networking topologies, secrets management, and provisioning workflows, see the centralized [Production Deployment Guide](docs/DEPLOY.md).

---

## 💾 Database Schema & Migrations

Orasaka's database schema is managed automatically via **Flyway Migrations**. The migration scripts reside inside [orasaka-identity/src/main/resources/db/migration/](file:///Users/oussamaabid/Documents/projects/orasaka/orasaka-identity/src/main/resources/db/migration/).

- **Startup Execution**: When the gateway boots up (either via `./ops/scripts/start.sh` or manually), Flyway automatically baselines the database and applies all pending versioned scripts (e.g. `V1__initial_schema.sql`, `V2__add_oauth_provider_to_users.sql`).
- **Schema Evolution Runbook**:
  - Never execute manual `ALTER TABLE` or `CREATE TABLE` commands.
  - To evolve the database schema, add a new versioned file (e.g. `V3__your_description.sql`) under the migration directory.
  - Flyway will pick up and execute the new migration on the next application boot.
...
| `/api/v1/jobs`             | `POST` | Submit a job asynchronously (returns HTTP 202)    |
| `/api/v1/jobs`             | `GET`  | Get paginated list of jobs for authenticated user |
| `/api/v1/jobs/{id}`        | `GET`  | Fetch single job execution status and results     |
| `/api/v1/jobs/stream`      | `GET`  | SSE real-time job status notifications stream     |
...
│   ├── local/http/orasaka.http # REST/GraphQL test files
│   ├── local/postgres/init/    # Schema and seed SQL
│   └── local/scripts/          # start.sh, stop.sh
│   └── deploy/                 # Production infrastructure files
...
| :------------------------------------------------ | :----------------------------------------------------------- |
| [Orasaka 101: First Steps](docs/ORASAKA101.md)    | Master developer onboarding, core concepts, and architecture guide |
...
| [ADR Log](docs/CONTEXT.md)                        | 24 Architectural Decision Records with rationale             |
...
- **No `Orasaka` prefix** — use `Engine`, not `OrasakaEngine` (with zero exceptions)
...
| **Unidirectional Dependencies** | Gateway → Core ✅ <br/> Core → Gateway ❌ <br/> Tools → Identity ❌               |
