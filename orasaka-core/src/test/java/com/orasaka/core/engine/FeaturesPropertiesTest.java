...
class FeaturesPropertiesTest {
    ...
    void immutableCopy() {
      var config =
          new FeaturesProperties.FeatureConfig(
              true, "Text Chat", "chat", "/api/v1/chat", "POST", null);
      var props = new FeaturesProperties(Map.of("orasaka.core.chat.text", config));
      assertThrows(UnsupportedOperationException.class, () -> props.features().put("new", config));
    }
  }
  ...
  class FeatureConfigTests {
    ...
    void validConfig() {
      var config =
          new FeaturesProperties.FeatureConfig(
              true, "Image Gen", "image", "/api/v1/image", "POST", "{\"prompt\":\"${p}\"}");
    ...
    void nullLabel() {
      assertThrows(
          ...
          () -> new FeaturesProperties.FeatureConfig(true, null, "icon", "/uri", "POST", null));
    }
    ...
    void blankLabel() {
      assertThrows(
          ...
          () -> new FeaturesProperties.FeatureConfig(true, "  ", "icon", "/uri", "POST", null));
    }
    ...
    void nullIcon() {
      assertThrows(
          NullPointerException.class,
          () -> new FeaturesProperties.FeatureConfig(true, "Label", null, "/uri", "POST", null));
    }
    ...
    void blankIcon() {
      assertThrows(
          IllegalArgumentException.class,
          () -> new FeaturesProperties.FeatureConfig(true, "Label", "  ", "/uri", "POST", null));
    }
    ...
    void nullUriPath() {
      assertThrows(
          NullPointerException.class,
          () -> new FeaturesProperties.FeatureConfig(true, "Label", "icon", null, "POST", null));
    }
    ...
    void disabledFeature() {
      var config =
          new FeaturesProperties.FeatureConfig(
              false, "Speech", "mic", "/api/v1/speech", "POST", null);
