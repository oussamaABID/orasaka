...
class MemoryResolverTest {
  ...
  class SessionResolution {
    ...
    @Test
    @DisplayName("null conversationId returns transient memory")
    void nullIdTransient() {
      var resolver = new MemoryResolver();
      var mem1 = resolver.resolve(null);
      var mem2 = resolver.resolve(null);
      // Transient memories are distinct instances
      assertNotSame(mem1, mem2);
    }

    @Test
    @DisplayName("blank conversationId returns transient memory")
    void blankIdTransient() {
      var resolver = new MemoryResolver();
      var mem1 = resolver.resolve("  ");
      var mem2 = resolver.resolve("  ");
      assertNotSame(mem1, mem2);
    }
    ...
    void sameIdSameMemory() {
      var resolver = new MemoryResolver();
      var mem1 = resolver.resolve("conv-1");
      var mem2 = resolver.resolve("conv-1");
  ...
  class InMemoryOps {
    ...
    void getDefensiveCopy() {
      var resolver = new MemoryResolver();
      var mem = resolver.resolve("test");
      mem.add("test", List.of(new UserMessage("hello")));
  ...
  class Purge {
    ...
    void purgeRemovesMemory() {
      var resolver = new MemoryResolver();
      var mem1 = resolver.resolve("conv-1");
      mem1.add("conv-1", List.of(new UserMessage("hello")));

      resolver.purge("conv-1");
      var mem2 = resolver.resolve("conv-1");
      // Should be a new instance, not the same
      assertNotSame(mem1, mem2);
      assertTrue(mem2.get("conv-1").isEmpty());
    }
