...
class EnginePipelineBridgeTest {
  ...
  class CompileContext {
    ...
    void compilesWithoutPipeline() {
      ...
      var result = enginePipelineBridge.compileContext(request, null, "ollama", List.of());
    ...
    void compilesWithActivePipeline() {
      var pipeline = mock(OrchestrationPipeline.class);
      var ctx = new Context("user1", "conv-123", Map.of(), Set.of());
      var promptCtx = new PromptContext("refined question", Map.of()).withRoutedProvider("ollama");
    ...
    void keepsOpenAiProvider() {
      ...
      var result = enginePipelineBridge.compileContext(request, null, "openai", List.of());
    ...
    void throwsOnMissingOllamaModel() {
      ...
      assertThrows(
          CoreException.class,
          () -> enginePipelineBridge.compileContext(request, null, "ollama", emptyInterceptors));
    }
    ...
    void mapsOpenAiChatOptionsToOllama() {
      ...
      var pipeline = mock(OrchestrationPipeline.class);
      var promptCtx = new PromptContext("test", Map.of()).withRoutedProvider("ollama");
      when(pipeline.process(anyString(), any())).thenReturn(promptCtx);

      var result = enginePipelineBridge.compileContext(request, pipeline, "ollama", List.of());
    ...
    void defaultOptionsPerProvider() {
      ...
      var result1 = enginePipelineBridge.compileContext(request1, null, "ollama", List.of());
      ...
      var result2 = enginePipelineBridge.compileContext(request1, null, "openai", List.of());
    ...
    void appliesInterceptors() {
      ...
      var result =
          enginePipelineBridge.compileContext(request, null, "ollama", List.of(mockInterceptor));
    ...
    void compilesWithMcpServers() {
      var request =
          new InternalChatRequest(
              "test prompt", List.of(), null, new Context("user1", "conv-123", Map.of(), Set.of()));
      ...
      var userServer =
          new UserMcpServer(1, "user1", "user-label", "http://user/mcp", "user-token", true);

      when(platformMcpServerProvider.getActivePlatformMcpServers())
          .thenReturn(List.of(platformServer, platformLocalServer));
      when(userMcpServerProvider.getActiveUserMcpServers("user1")).thenReturn(List.of(userServer));
