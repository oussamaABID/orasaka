...
public class LocalAudioExtractor {
  ...
  public byte[] extractAudio(byte[] videoBytes) {
    ...
    try {
      tempVideoFile = File.createTempFile("video_input_", ".mp4");
      tempAudioFile = File.createTempFile("audio_output_", ".mp3");
      tempLogFile = File.createTempFile("ffmpeg_log_", ".log");
      ...
      if (exitCode != 0) {
        String logContent = "";
        try {
          logContent = Files.readString(tempLogFile.toPath());
        } catch (IOException ex) {
          logger.warn("Failed to read FFmpeg log file: {}", tempLogFile.getAbsolutePath(), ex);
        }
        throw new IllegalStateException(
  ...
  private void cleanupFile(File file) {
    if (file != null && file.exists()) {
      try {
        ...
        if (!deleted) {
          logger.warn("Failed to delete temporary media file: {}", file.getAbsolutePath());
        }
